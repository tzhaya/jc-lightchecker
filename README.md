...
GitHub Pages (https://tzhaya.github.io/jc-lightchecker/) を利用して公開中です。過去14日分の応答状況を、登録されている機関リポジトリごとに確認できます。
...
- 12h / 24h / 7d / all の応答時間グラフ（SERVER_ERROR / TIMEOUT / UNKNOWN の点は円ではなく × で表示し、正常応答との取り違えを防止）
...
同じメタデータを配信していても、描画のしかたは各プラットフォームが決めます。2026-08-01 時点で確認した挙動は次のとおりです。

| | 画像 | タイトル | 稼働状況（description） |
| --- | --- | --- | --- |
| Slack | 表示 | 表示 | 表示 |
| Facebook（Sharing Debugger） | 表示 | 表示 | 表示 |
| X | 表示 | 画像に重ねて表示 | **表示されない** |

**X ではカードに稼働状況が出ません。** X のカードは画像・タイトル・ドメイン名だけで構成され、description を描画する領域がありません。メタデータ自体は正しく配信されています（Facebook の Sharing Debugger が `twitter:description` を含む全タグを読み取れることを確認済み）。ページ側から変えられる範囲の外の挙動です。

そのため X の閲覧者には、稼働状況はカードからは伝わりません。ただし「非公式・個人運用のダッシュボードです」という免責は画像そのものに含めてあるため、X でも必ず伝わります。

タイトルは静的で、書き換えていません。X にも稼働状況を出したい場合は `og:title` / `twitter:title` を動的にする必要がありますが、その場合は両方を同じ書き換えで同時に更新し、値が一致することをテストで固定してください。
...
- 2026-08-04: ダッシュボードをブラウザのタブから識別しやすくするため、パルス波形をモチーフにした SVG favicon を追加。
- 2026-08-01: 初期の設計案 `jc-lightchecker.md` を `notes/` へ移し、現行実装と異なる旨（`schedule` 廃止など）を冒頭に明記した。
- 2026-08-01: SNS や Slack のリンクプレビュー向けに OGP / X Card のメタデータと静的画像 `docs/ogp.png` を追加。稼働状況は description のみを動的に書き換える方式とし、画像は書き換えない（可変な画像を毎回コミットすると Git 履歴が単調増加し、`git gc` では回収できないため）。あわせて `docs/latest.json`、`docs/history.jsonl`、`docs/index.html` の書き込みを一時ファイルを経由した安全な置き換え（失敗時は元ファイルを残す）に変更した。

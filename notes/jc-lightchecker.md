...
> **これは初期の設計案であり、現行実装とは異なります。** 経緯の記録として残しているもので、仕様の参照先ではありません。現在の仕様は [README.md](../README.md) と実際のコードを参照してください。
>
> 特に次の点はすでに変わっています。
>
> - 本書の `check.yml` は `schedule: "*/15 * * * *"` を持つが、**現行は `workflow_dispatch` 専用**。実行間隔は Cloudflare Worker の cron（`7,22,37,52 * * * *`）が制御する（GitHub の `schedule` は高負荷時に遅延・欠落しうるため）
> - 本書の `actions/checkout@v4` は、現行では v7 を commit SHA でピン留めしている
